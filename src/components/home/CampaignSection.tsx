...
export default function CampaignSection() {
  ...
  const urgentCampaign = campaigns.find((c) => c?.priority === true);
  const progress = urgentCampaign
    ? (urgentCampaign.donatedAmount / urgentCampaign.amount) * 100
    : 0; // fallback if no urgent campaign

  if (!mounted) return null;

  console.log("urgentCampaign", urgentCampaign);
  return (
          ...
          >
            ...
            {urgentCampaign ? (
              ...
              <>
                ...
                <p className="text-sm sm:text-base md:text-base text-muted-foreground mb-6 md:mb-8 break-words">
                  Currently, there are no urgent medical campaigns. Please check
                  back later to see how you can help support healthcare
              ...
              >
                ...
                <div className="flex justify-between text-xs sm:text-sm">
                  ...
                  <span className="font-semibold text-primary text-xs sm:text-sm">
                    ${urgentCampaign?.donatedAmount.toFixed(2)} / $
                    {urgentCampaign?.amount.toFixed(2)}
                  </span>
                </div>
              </motion.div>
            </motion.div>
