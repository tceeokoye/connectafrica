...
export async function GET(req: NextRequest) {
  try {
    ...
    const monthlyDonationStats = await donationsCollection.aggregate([
      { $match: { status: "completed", createdAt: { $gte: thirtyDaysAgo } } },
      { $group: { _id: null, monthlyAmount: { $sum: "$amount" }, monthlyCount: { $sum: 1 } } }
    ]).toArray();
    ...
    const previousMonthDonations = await donationsCollection.aggregate([
    ...
    const donationGrowth = previousMonthAmount > 0 
      ? (((monthlyDonations - previousMonthAmount) / previousMonthAmount) * 100).toFixed(1)
      : 0;
    ...
    const subscriberGrowth = previousMonthSubscribers > 0
    ...
    return NextResponse.json(
      { success: false, message: "Failed to fetch dashboard stats", error: err?.message },
      { status: 500 }
    );
  }
}
