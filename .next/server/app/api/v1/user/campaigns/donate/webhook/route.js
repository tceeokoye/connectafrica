"use strict";(()=>{var e={};e.id=9111,e.ids=[9111],e.modules={38013:e=>{e.exports=require("mongodb")},20399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},61282:e=>{e.exports=require("child_process")},84770:e=>{e.exports=require("crypto")},80665:e=>{e.exports=require("dns")},17702:e=>{e.exports=require("events")},92048:e=>{e.exports=require("fs")},32615:e=>{e.exports=require("http")},35240:e=>{e.exports=require("https")},98216:e=>{e.exports=require("net")},19801:e=>{e.exports=require("os")},55315:e=>{e.exports=require("path")},76162:e=>{e.exports=require("stream")},82452:e=>{e.exports=require("tls")},17360:e=>{e.exports=require("url")},21764:e=>{e.exports=require("util")},71568:e=>{e.exports=require("zlib")},94885:(e,o,t)=>{t.r(o),t.d(o,{originalPathname:()=>y,patchFetch:()=>b,requestAsyncStorage:()=>f,routeModule:()=>m,serverHooks:()=>h,staticGenerationAsyncStorage:()=>x});var r={};t.r(r),t.d(r,{POST:()=>g});var n=t(49303),i=t(88716),a=t(60670),s=t(87070),p=t(75748),d=t(38013),l=t(55245),c=t(98800);async function u(e,o){try{let t=e.headers.get("paypal-transmission-id"),r=e.headers.get("paypal-transmission-time"),n=e.headers.get("paypal-cert-url"),i=e.headers.get("paypal-transmission-sig"),a=process.env.PAYPAL_WEBHOOK_ID;if(!t||!r||!n||!i||!a)return console.error("Missing PayPal webhook headers"),!1;let s=Buffer.from(`${process.env.PAYPAL_CLIENT_ID}:${process.env.PAYPAL_SECRET}`).toString("base64"),p=await fetch(`${process.env.PAYPAL_API_URL}/v1/oauth2/token`,{method:"POST",headers:{Authorization:`Basic ${s}`,"Content-Type":"application/x-www-form-urlencoded"},body:"grant_type=client_credentials"}),d=(await p.json()).access_token,l=await fetch(`${process.env.PAYPAL_API_URL}/v1/notifications/verify-webhook-signature`,{method:"POST",headers:{Authorization:`Bearer ${d}`,"Content-Type":"application/json"},body:JSON.stringify({transmission_id:t,transmission_time:r,cert_url:n,auth_algo:"SHA256withRSA",transmission_sig:i,webhook_id:a,webhook_event:JSON.parse(o)})}),c=await l.json();return"SUCCESS"===c.verification_status}catch(e){return console.error("PayPal webhook verification error:",e),!1}}async function g(e){let o="";try{console.log("\n\uD83D\uDD14 PAYPAL WEBHOOK RECEIVED"),o=await e.text();let t=await u(e,o);if(console.log("Signature Valid:",t),!t)return console.error("❌ Signature verification failed - rejecting webhook"),s.NextResponse.json({success:!1,message:"Invalid signature"},{status:401});let r=JSON.parse(o);if(console.log("\uD83D\uDCE8 Event Type:",r.event_type),"CHECKOUT.ORDER.COMPLETED"!==r.event_type)return console.log("⏭️ Ignoring event type:",r.event_type),s.NextResponse.json({message:"Event type ignored"});let n=r.resource;if(!n||!n.id)return console.error("❌ Missing order data"),s.NextResponse.json({success:!1,message:"Missing order data"},{status:400});console.log("\uD83D\uDCB3 PayPal Order ID:",n.id);let i=(await p.Z).db("connect_africa"),a=i.collection("donations"),g=i.collection("campaigns"),m=await a.findOne({paypalOrderId:n.id,status:{$ne:"completed"}});if(console.log("\uD83D\uDD0D Donation lookup result:",m?"✅ Found":"❌ Not found"),!m)return console.error("❌ Donation not found or already processed. Order ID:",n.id),s.NextResponse.json({success:!0,message:"Donation not found but webhook processed"});let f=n.purchase_units?.[0]?.amount?.value||m.amount;if(console.log("\uD83D\uDCCB Donation Details:",{paypalOrderId:m.paypalOrderId,status:m.status,amount:m.amount,campaignId:m.campaignId}),Math.abs(Number(f)-Number(m.amount))>.01)return console.error("❌ Amount mismatch:",{expected:m.amount,received:f}),s.NextResponse.json({success:!1,message:"Amount mismatch"},{status:400});if(!m.campaignId)return console.error("❌ Invalid campaign ID in donation"),s.NextResponse.json({success:!1,message:"Invalid campaign ID"},{status:400});let x=await g.findOne({_id:new d.ObjectId(m.campaignId)});if(console.log("\uD83C\uDFAF Campaign lookup:",x?`✅ Found: ${x.title}`:"❌ Not found"),!x)return s.NextResponse.json({success:!1,message:"Campaign not found"},{status:404});console.log("\uD83D\uDD04 Updating donation status to completed...");let h=await a.updateOne({paypalOrderId:n.id},{$set:{status:"completed",completedAt:new Date,amountPaid:Number(f),paypalStatus:n.status}});if(console.log("✅ Donation updated:",{modifiedCount:h.modifiedCount,matchedCount:h.matchedCount}),0===h.modifiedCount)return console.error("❌ Failed to update campaign donation:",n.id),s.NextResponse.json({success:!1,message:"Failed to update donation"},{status:500});let y=(x.donatedAmount||0)+Number(f),b=y>=x.amount?"completed":"inprogress";console.log("\uD83D\uDCB5 Campaign Update:",{previousDonatedAmount:x.donatedAmount||0,newDonation:Number(f),totalNow:y,targetAmount:x.amount,newStatus:b});let v=await g.updateOne({_id:x._id},{$set:{donatedAmount:y,status:b,updatedAt:new Date},$inc:{volunteers:1}});console.log("✅ Campaign updated:",{modifiedCount:v.modifiedCount,donatedAmount:y});try{console.log("\uD83D\uDCE7 Sending confirmation email to:",m.email);let e=l.createTransport({service:"gmail",auth:{user:process.env.GMAIL_USER,pass:process.env.GMAIL_PASS}}),{subject:o,html:t}=(0,c.aT)({name:m.firstName?`${m.firstName} ${m.lastName}`:"Friend",reference:m.paypalOrderId||n.id,amountPaid:Number(f),donationType:"one-time",designation:x.title});await e.sendMail({from:process.env.GMAIL_USER,to:m.email,subject:o,html:t}),console.log("✅ Confirmation email sent successfully to:",m.email)}catch(e){console.error("⚠️ Warning: Failed to send email:",e)}return console.log("\n✅ WEBHOOK PROCESSING COMPLETE"),console.log("   Order ID:",n.id),console.log("   Amount: $"+f),console.log("   Campaign:",x.title),console.log("   New Total: $"+y),console.log("\n"),s.NextResponse.json({success:!0,message:"Campaign donation processed successfully",reference:n.id})}catch(e){return console.error("\n❌ WEBHOOK ERROR:",e.message),console.error("Stack:",e.stack),s.NextResponse.json({success:!0,message:"Webhook received"},{status:200})}}let m=new n.AppRouteRouteModule({definition:{kind:i.x.APP_ROUTE,page:"/api/v1/user/campaigns/donate/webhook/route",pathname:"/api/v1/user/campaigns/donate/webhook",filename:"route",bundlePath:"app/api/v1/user/campaigns/donate/webhook/route"},resolvedPagePath:"C:\\Users\\HP\\OneDrive\\Desktop\\project2\\connect africa\\src\\app\\api\\v1\\user\\campaigns\\donate\\webhook\\route.ts",nextConfigOutput:"",userland:r}),{requestAsyncStorage:f,staticGenerationAsyncStorage:x,serverHooks:h}=m,y="/api/v1/user/campaigns/donate/webhook/route";function b(){return(0,a.patchFetch)({serverHooks:h,staticGenerationAsyncStorage:x})}},75748:(e,o,t)=>{t.d(o,{Z:()=>i});var r=t(38013);let n=process.env.MONGODB_URI;if(!n)throw Error("Please define MONGODB_URI in .env");let i=new r.MongoClient(n).connect()},98800:(e,o,t)=>{function r({name:e,reference:o,amountPaid:t,donationType:r,designation:n}){return{subject:`Thank you for your donation — ${o}`,html:`
  ...
  `}}t.d(o,{RY:()=>a,X:()=>n,aT:()=>r,pF:()=>i})}};var o=require("../../../../../../../webpack-runtime.js");o.C(e);var t=e=>o(o.s=e),r=o.X(0,[9276,5972,5245],()=>t(94885));module.exports=r})();